...
public class SearchLight : MonoBehaviour
{
    ...
    void Update()
    {
        bool onPlayerEnter = Physics2D.CircleCast(transform.position, transform.localScale.x * 0.5f, transform.forward, Mathf.Infinity, 1 << 8);

        foreach (var r in Physics2D.CircleCastAll(transform.position, transform.localScale.x * 0.5f, transform.forward, Mathf.Infinity, 1 << 10))
        {
